    ...
    switch (addr->getKind()) {
      ...
      case ValueKind::StructElementAddrInst: {
        ...
        if (SEA->getStructDecl()->getName().is("_SwiftArrayBodyStorage") &&
            (SEA->getField()->getName().isCompoundName() ||
             !SEA->getField()->getBaseIdentifier().is("count"))) {
          return false;
        }
        addr = SEA->getOperand();
        break;
      ...
      case ValueKind::RefElementAddrInst: {
        ...
        if (className.is("__RawDictionaryStorage") ||
          ...
          assert(REA->getField()->getName().isSimpleName());
          Identifier fieldName = REA->getField()->getBaseIdentifier();
